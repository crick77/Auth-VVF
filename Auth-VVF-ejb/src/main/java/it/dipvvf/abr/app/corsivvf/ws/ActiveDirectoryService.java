...
public class ActiveDirectoryService {
    ...
    String adPassword;
    boolean doSimulation;
    String[] serverList;
    Control[] connCtls = new Control[]{new FastBindConnectionControl()};

    @Resource(lookup = "doSimulation", type = Boolean.class)
    void setDoSimulation(Boolean doSimulation) {
        doSimulation = (doSimulation == null) ? Boolean.FALSE : doSimulation;
        this.doSimulation = doSimulation;
        System.out.println("ActiveDirectory users: "+((this.doSimulation) ? "SIMULATED" : "REAL"));
    }
    ...
    public boolean checkUser(@WebParam(name = "username") String username, @WebParam(name = "password") String password) {
        if(doSimulation) {
            return UserDatabase.connect().checkUser(username, password);
        }
        else {
    ...
    public String[] getUserGroups(@WebParam(name = "username") String username) {
        if(doSimulation) {
            return UserDatabase.connect().getUserGroups(username);
        }
        else {
